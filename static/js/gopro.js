...
function process(media) {
    ...
    media.forEach((d, i) => {d.captured_at = new Date(d.captured_at);
                             d.created_at = new Date(d.created_at);
                             d.file_mb = d.file_size / (1024*1024);
                            });

    const nestByDate = d3.nest()
          .key(d => d3.timeDay(d.captured_at));

    const medium = crossfilter(media),
          all = medium.groupAll(),
          date = medium.dimension(d => d.captured_at),
          dates = date.group(d3.timeDay),
          filesize = medium.dimension(d => d.file_mb),
          filesizes = filesize.group();

    const allDates = d3.extent(media, d => d.captured_at),
          allSizes = d3.extent(media, d => d.file_mb);
    ...
    function mediaList(div) {
        ...
        div.each(function() {
            var date = d3.select(this).selectAll(".date")
                .data(mediaByDate, function(d) { return d.key; });

            date.enter().append("div")
                .attr("class", "date")
                .append("div")
                .attr("class", "day")
                .text(function(d) { return formatDate(d.values[0].captured_at); });
            ...
            mEnter.append("div")
                .attr("class", "time")
                .text(function(d) { return formatTime(d.captured_at); });
            mEnter.append("img")
                .attr("class", "thumb")
